...
fun UnitConverter() {
    ...
    fun convertUnits(){
        // ?:  - elvis operator
        val inputValueDouble = inputValue.toDoubleOrNull() ?: 0.0 //gives no. or if null then gives 0.0
        val result = (inputValueDouble * conversionFactor.value * 100.0/oConversionFactor.value).roundToInt()/100.0 //doesn't give long weird numbers.
    ...
    Column(
        ...
        Text("Unit Converter", style = customTextStyle) // Minimum req. is this text field.
        Spacer(modifier = Modifier.height(16.dp))
        OutlinedTextField(value = inputValue, onValueChange = {
            inputValue = it
            convertUnits()  //Added so that if change is made in dialogue box it updates the result without any click
        ...
        Row { //Row is parent of Box
            ...
            Box {
                ...
                DropdownMenu(expanded = iExpanded, onDismissRequest = { iExpanded = false }) {
                    DropdownMenuItem(text = { Text("Centimeters") },
                        onClick = {
                            iExpanded = false // dropdownmenu is closed
            ...
            //Output Button
            Box {
                Button(onClick = { oExpanded = true }) {   //Box is Parent of Button
